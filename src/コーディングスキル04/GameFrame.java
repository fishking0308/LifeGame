...
public class GameFrame extends JFrame implements Runnable{
	...
	private void addButtonsOnPanel(JPanel panel){
		...
		ActionListener generateRandamBtnAction = new ActionListener(){
			public void actionPerformed(ActionEvent event){
				//ランダムパターン生成
				CellPattern.patternGenerator(cell_matrix, CellPattern.Pattern.Randam);
			}
		};
		...
		//スペーサー
		btnSources.put("ランダム", generateRandamBtnAction);
		...
		for(Map.Entry<String, ActionListener>  btnSrc : btnSources.entrySet()){
			...
			button.setBounds(10 + i * 80,panel.getBounds().y, 100, 60);
